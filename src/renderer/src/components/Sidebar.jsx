  ...
  const Sidebar = ({
    ...
    const fetchNewCardData = async () => {
      const cardTitle = titleInputRef.current?.getTitle().trim();
        ...
        if (!cardTitle) {
            showNotification("Error", "Card title is required"); // Replaced alert
            return;
        }
      ...
      try {
        const response = ('http://127.0.0.1:5000/api/new-card', {
          method: 'POST',
    ...
    const handleFileUpload = async (event) => {
      ...
      if (!file) {
          showNotification("Wait", "Please select a file first!");
        return;
      }
